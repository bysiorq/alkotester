...
CONFIG = {
    "screen_width": 720,
    "screen_height": 1280,   # pion
    "fullscreen": False,     # <- na razie bez fullscreen żeby nie uwalić wyświetlacza
    "rotate_180": False,

    "camera_main_size": (1280, 720),
    "camera_fps": 30,
...
class FaceDB:
    """
    Prosta baza twarzy:
      data/employees.json          -> lista pracowników (id, name, pin)
      data/faces/<emp_id>/*.jpg    -> obrazy twarzy
      data/index/<emp_id>.npz      -> deskryptory ORB
    """
    def __init__(self, faces_dir, index_dir, employees_json):
        ...
        # Klasyfikator Haar bez cv2.data (bo na Pi różne wersje OpenCV)
        self.cascade = self._load_haar_cascade()

        self.matcher = cv2.BFMatcher(cv2.NORM_HAMMING, crossCheck=True)

        self.index = {}  # emp_id -> [np.ndarray deskryptorów]
        self._load_index()
    ...
    def _load_haar_cascade(self):
        ...
        for p in candidates:
            if os.path.exists(p):
                ...
                if not c.empty():
                    return c
        raise RuntimeError("Nie mogę znaleźć pliku haarcascade_frontalface_default.xml. Zainstaluj pakiet `opencv-data`.")
...
class KeypadDialog(QtWidgets.QDialog):
    def __init__(self, parent=None, title="Wprowadź PIN"):
        ...
        self.label.setStyleSheet("font-size: 28px; font-weight: 600; color:white;")
        ...
        self.edit.setStyleSheet("font-size: 32px; padding: 8px; border-radius: 12px; background: #222; color: white;")
        layout.addWidget(self.edit)

        grid = QtWidgets.QGridLayout()
        btn_style = "font-size:26px; padding:16px; border-radius:16px; background:#333; color:white;"
        buttons = [
            ("1", 0, 0), ("2", 0, 1), ("3", 0, 2),
            ("4", 1, 0), ("5", 1, 1), ("6", 1, 2),
            ("7", 2, 0), ("8", 2, 1), ("9", 2, 2),
...
class CameraWorker(QtCore.QObject):
    ...
    def grab(self):
        if self.picam is None:
            return
        try:
            frame = self.picam.capture_array("main")  # RGB (H,W,3)
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def training_progress(self, title):
        ...
        threading.Thread(
            target=self.facedb.train_reindex,
    ...
    def on_frame(self, frame_rgb: np.ndarray):
        # zapisz ostatnią klatkę jako BGR
        ...
        # jeśli kamera daje poziomo (1280x720) a ekran jest pionowy, obróć o 90°
        h, w, _ = disp.shape
        if w > h:
        ...
        # auto przejście z idle -> detect jeśli widać twarz
        if self.state == "idle":
    ...
    def _calibrate_mq3_start(self):
        ...
        t.start()
        QtCore.QTimer.singleShot(1200, lambda: self.set_message(now_str(), "Podejdź bliżej"))
